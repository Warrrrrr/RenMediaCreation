...
def _extract_approved_strategies(strategy_map):
    """Extract only the strategies explicitly present in the approved map."""
...
def _build_strategy_instruction(strategy_map):
    ...
    if not approved:
        return "NO APPROVED STRATEGIES WERE IDENTIFIED.\nDo not invent or evaluate strategies."
    return "\n".join(f"- {strategy}" for strategy in approved)
...
If another strategy appears in the script, do NOT add it to the selected strategy list.
...
Finding a word, phrase or concept associated with a strategy does NOT mean the
strategy was successfully executed. Evaluate execution, not keyword presence.
...
Do not flag ordinary first-person narration unless it falsely implies real experience by the creator.
...
STATUS RULES:
PASS = no critical blocking problem.
WARNING = issues worth reviewing but not necessarily blocking.
CRITICAL = serious factual, ethical, fabrication, or structural failure.

Each approved strategy must produce exactly one strategy evaluation object.
Do NOT include strategies outside the approved list.
...
def validate_script(
    ...
    """Validate a generated script against approved strategies and source claims."""
    approved_keys = _extract_approved_strategies(strategy_map)
    approved_text = _build_strategy_instruction(strategy_map)
    ...
    return _enforce_strategy_scope(result, approved_keys)
