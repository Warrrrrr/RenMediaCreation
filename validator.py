...
def _extract_approved_strategies(strategy_map):
    ...
    if isinstance(strategy_map, str):
        try:
            return _extract_approved_strategies(json.loads(strategy_map.strip()))
        except Exception:
            return re.findall(r'"(?:key|id|strategy_key|strategy_id)"\s*:\s*"([^"]+)"', strategy_map)
        ...
        for item in strategy_map:
            for value in _extract_approved_strategies(item):
                if value not in result:
                    result.append(value)
        return result
    if isinstance(strategy_map, str) and strategy_map.strip():
        return [strategy_map.strip()]
    return []
...
STRATEGY VALIDATION
For each approved strategy, evaluate actual execution, job completion, appropriateness and overuse.
Do not treat keyword presence as execution.
...
A claim is NOT covered merely because it is related to the topic or resembles a supplied claim.
Specific numbers, physiological mechanisms, causal explanations, prevalence statements, predictions, or claims about what people generally do/feel require their own support.
